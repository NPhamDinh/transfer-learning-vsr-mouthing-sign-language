...
train_lrw = DatasetFolder("./fg_lrw2/train", loader=loaderAug, extensions=("mp4"))
val_lrw = DatasetFolder("./fg_lrw2/val", loader=loader, extensions=("mp4"))
test_lrw = DatasetFolder("./fg_lrw2/test", loader=loader, extensions=("mp4"))
...
if __name__ == "__main__":
    ...
    train_sampler_lrw = torch.utils.data.distributed.DistributedSampler(train_lrw, num_replicas=num_gpus, rank=int(os.getenv('RANK', 0)))
    val_sampler_lrw = torch.utils.data.distributed.DistributedSampler(val_lrw, num_replicas=num_gpus, rank=int(os.getenv('RANK', 0)))
    test_sampler_lrw = torch.utils.data.distributed.DistributedSampler(test_lrw, num_replicas=num_gpus, rank=int(os.getenv('RANK', 0)))
    
    trainloader_lrw = DataLoader(train_lrw, shuffle=False, batch_size=64, num_workers=4, sampler=train_sampler_lrw)
    valloader_lrw = DataLoader(val_lrw, shuffle=False, batch_size=64, num_workers=4, sampler=val_sampler_lrw)
    testloader_lrw = DataLoader(test_lrw, shuffle=False, batch_size=64, num_workers=4, sampler=test_sampler_lrw)
    ...
    model_state_ddict = torch.load("./Weights/hpc_dgsglipsmglipsrlrw.pth")
    Model.load_state_dict(model_state_ddict)    
    ...
    def train(model):
        ...
        for i in range(epoch, 10000):
            ...
            zipped = zip(trainloader_glipsm, trainloader_glipsr, trainloader_lrw, trainloader_dgs)
            for j, ((glipsm_batch_X, glipsm_batch_y), (glipsr_batch_X, glipsr_batch_y), (lrw_batch_X, lrw_batch_y), (dgs_batch_X, dgs_batch_y)) in enumerate(zipped):
                ...
                lrw_batch_X = Variable(lrw_batch_X).to(device, dtype=torch.float)
                lrw_batch_y = Variable(lrw_batch_y).to(device)
                ...
                output_lrw = model.forward(lrw_batch_X, task_id = 3)
                loss_lrw = error(output_lrw, lrw_batch_y)

                loss = loss_dgs + loss_glipsm + loss_glipsr + loss_lrw
                loss.backward()
            ...
            }, "./ckpt2_hps_dgs-glipsm-glipsr-lrw.pt")
